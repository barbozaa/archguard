...
import { ParameterAnalysisRule } from '@domain/rules/base/parameter-analysis.rule.js';
...
class TestParameterAnalysisRule extends ParameterAnalysisRule {
  ...
  protected processMethod(method: MethodDeclaration, scope: import('@domain/rules/base/parameter-analysis.rule.js').ClassMemberScope): void {
    const params = method.getParameters();
    if (params.length > 2) {
      this.violations.push({
        rule: 'Test Rule',
        severity: 'info',
        message: `Method ${scope.className}.${method.getName()} has ${params.length} parameters`,
        file: scope.relativePath,
  ...
  protected processConstructor(constructor: ConstructorDeclaration, scope: import('@domain/rules/base/parameter-analysis.rule.js').ClassMemberScope): void {
    const params = constructor.getParameters();
    if (params.length > 2) {
      this.violations.push({
        rule: 'Test Rule',
        severity: 'info',
        message: `Constructor ${scope.className} has ${params.length} parameters`,
        file: scope.relativePath,
