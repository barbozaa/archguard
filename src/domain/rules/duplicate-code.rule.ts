...
import { Severity, Violation } from '@domain/types.js';
...
export class DuplicateCodeRule implements Rule {
  ...
  private isInsignificant(normalized: string): boolean {
    ...
    // Skip boilerplate: closing delimiters + single return statement + closing delimiters
    // These patterns arise naturally at the end of every method and carry no semantic meaning
    if (/^[}\]);]*return [^;]+;[}\]);]*$/.test(normalized)) return true;

    return false;
  }

  /**
   * SHA-256 hash (first 16 hex chars = 64 bits).
   * DJB2 on 32 bits had ~45% collision probability on 50k blocks (Birthday Paradox).
   * 64-bit prefix of SHA-256 gives P(collision) < 10^-9 for realistic project sizes.
   */
