import { DependencyGraph, Violation, ModuleCouplingMetrics, CouplingRiskAnalysis } from '@domain/types.js';
...
export class CouplingRiskAnalyzer {
  ...
  analyze(graph: DependencyGraph, violations: Violation[]): CouplingRiskAnalysis {
    ...
    for (const modulePath of modules) {
      ...
      const cycleCount = this.getCycleParticipation(modulePath, graph);
  ...
  /**
   * Count how many detected cycles involve this module.
   * Uses the dependency graph's cyclicGroups computed by GraphBuilder,
   * not the deleted circular-deps rule violation.
   */
  private getCycleParticipation(modulePath: string, graph: DependencyGraph): number {
    return graph.cyclicGroups.filter(cycle => cycle.includes(modulePath)).length;
  }
